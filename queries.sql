...
BEGIN
    RETURN QUERY
    SELECT tf.cod_album, tf.nome, tf.descricao FROM 
        ((SELECT gravacao.cod_album FROM gravacao WHERE cod_banda = banda) AS t1
            NATURAL JOIN album) AS tf 
    ORDER BY tf.cod_album;
END
...
BEGIN
    RETURN QUERY
    SELECT tf.cod_album, tf.nome, tf.descricao FROM 
        ((SELECT versao.cod_album FROM
            gravacao WHERE cod_gravadora = gravadora) AS t1
        NATURAL JOIN album) AS tf
    ORDER BY tf.cod_album;
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    WITH t1 AS (SELECT versao.cod_barras, gravacao.cod_album, versao.preco FROM
                gravacao NATURAL JOIN versao WHERE cod_banda = banda)
    SELECT tf.cod_barras, tf.nome, tf.preco FROM 
        (t1 NATURAL JOIN album) AS tf
    WHERE tf.preco = (SELECT MAX(t1.preco) FROM t1);
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT tf.cod_barras, tf.nome, tf.preco FROM 
        (t1 NATURAL JOIN album) AS tf
    WHERE tf.preco = (SELECT MIN(t1.preco) FROM t1);
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    SELECT versao.cod_barras, versao.preco FROM
        versao 
    WHERE cod_album = album AND promocao = TRUE;
END
$func$ LANGUAGE plpgsql;

--RD5: Informar versões de um álbum cujas quantidades disponíveis estão abaixo de uma certa quantidade mínima

CREATE OR REPLACE FUNCTION list_low_stock(album int, qtd_minima int) RETURNS
TABLE (
    cod_barras   bigint,
    preco        numeric
)
...
BEGIN
    ...
    SELECT versao.cod_barras, versao.preco FROM
        versao
    WHERE qtd_disponivel <= qtd_minima AND versao.cod_album = album;
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    SELECT album.cod_album, album.nome FROM
        album
    WHERE genero1 = genero OR genero2 = subgenero;
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    SELECT versao.cod_gravadora, sum(qtd_vendida) FROM
        versao
    GROUP BY versao.cod_gravadora 
...
BEGIN
    RETURN QUERY
    SELECT tf.cod_banda, AVG(tf.avaliacao) as media FROM
        (album NATURAL JOIN gravacao) AS tf
    GROUP BY tf.cod_banda
    ORDER BY media DESC;
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT tf.cod_barras, tf.nome, tf.preco FROM 
        (versao 
            NATURAL JOIN (SELECT t1.cod_album, album.nome FROM
                (SELECT cod_album FROM
                    gravacao
                WHERE cod_banda = banda) as t1
            NATURAL JOIN album) AS t2) AS tf
    WHERE tf.tipo_versao = tipo;
END
$func$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
    SELECT album.cod_album, album.nome, album.descricao FROM
        album
    WHERE data_publicacao >= de AND data_publicacao <= ate;
END
$func$ LANGUAGE plpgsql;
