	...
	BEGIN
        ...
        ELSE
            ...
            IF (SELECT SUM(qtd_disponivel) FROM versao GROUP BY NEW.cod_album) >= TG_ARGV[1]::bigint THEN
            	RETURN NULL;
            ELSE
            	RAISE NOTICE 'Quantidade disponível do álbum % (Cod. %) abaixo do valor mínimo.', (SELECT nome FROM album WHERE cod_album = NEW.cod_album), NEW.cod_album;
                IF (SELECT SUM(qtd_disponivel) FROM versao GROUP BY NEW.cod_gravadora) >= TG_ARGV[2]::bigint THEN
                	RETURN NULL;
                ELSE
                	RAISE NOTICE 'Quantidade disponível da gravadora % (Cod. %) abaixo do valor mínimo.', (SELECT nome FROM gravadora WHERE cod_identificacao = NEW.cod_gravadora), NEW.cod_gravadora;
                    RETURN NULL;
                END IF;
            END IF;
        END IF;
